...
update_bashrc() {
    ...
    local export_line="export PATH=\$HOME/.local/bin/$PROGRAM_NAME:\$PATH"
    local bash_completion="\$HOME/.bash_completion.d/$PROGRAM_NAME"
    ...
    if ! grep -q "$bash_completion" "$rc_file" 2>/dev/null; then
        echo "source $bash_completion" >> "$rc_file" || echo "$err_msg"
    fi
}
...
main() {
    ...
    bold "To use $PROGRAM_NAME in this session, run:"
    bold "  source ~/.bashrc"
    bold "Or open a new terminal."
}
