...
namespace
{
const quint32 c_mailRequestIntervalMs = Examples::validConfig().mailRequestIntervalMs;
...
void DaemonQtTest::status_data()
{
    ...
    {
        MockPersistentStorage::TestData testData = happyPathStorageTestData();
        testData.readConfiguration = Result<Configuration>::error("error");

        IDaemon::Status status = Examples::validDaemonStatus();
        status.isMonitoringActivated = false;
        status.configuration = {};

        QTest::newRow("PersistentStorage::readConfiguration failed") << testData << status << false;
    }
...
{
    QTest::addColumn<MockPersistentStorage::TestData>("storageTestData");
    QTest::addColumn<bool>("isMonitoringActivated");
    QTest::addColumn<Configuration>("config");
    QTest::addColumn<bool>("success");
    QTest::addColumn<bool>("writeErrorLogMessageCalled");

    {
        QTest::newRow("Happy path")
            << happyPathStorageTestData()
            << false
            << Examples::validConfig()
            << true
            << false;
    }
...

    {
        ...
        QTest::newRow("Invalid PersistentStorage")
            ...
            << Examples::validConfig()
            << false
            << false; // because storage is not valid and we can't write error log message
    }
...

    {
        ...
        QTest::newRow("Invalid config (password)")
            << happyPathStorageTestData()
            << false
            << config
            << false
...

    {
        ...
        testData.writeConfiguration = "error";

        QTest::newRow("PersistentStorage::writeConfiguration failed")
            << testData
            << false
            << Examples::validConfig()
            << false
...
void DaemonQtTest::setup()
{
    // Assign
    QFETCH(MockPersistentStorage::TestData, storageTestData);
    QFETCH(bool, isMonitoringActivated);
    QFETCH(Configuration, config);
    QFETCH(bool, success);
    QFETCH(bool, writeErrorLogMessageCalled);
    ...
    // Assert
    QCOMPARE(result.isEmpty(), success);
    QCOMPARE(mockStorage->m_testData.writeErrorLogMessageCalled, writeErrorLogMessageCalled);
}
...
void DaemonQtTest::startMonitoring_data()
{
    ...
    {
        MockPersistentStorage::TestData storageTestData = happyPathStorageTestData();
        storageTestData.readConfiguration = Result<Configuration>::error("error");
        ...
        QTest::newRow("PersistentStorage::readConfiguration failed")
            << storageTestData
            << happyPathMailClientTestData()
            << happyPathNotificationManagerTestData()
            << result;
...
void DaemonQtTest::fetchMailboxes_data()
{
    ...
    {
        QTest::newRow("Happy path") << happyPathMailClientTestData() << true << Examples::validMailboxes();
    }

    {
        ...
        QTest::newRow("Invalid MailClient") << data << false << QStringList{};
    }

    {
        ...
        QTest::newRow("MailClient::fetchMailboxes failed") << data << false << QStringList{};
    }

    {
        ...
        QTest::newRow("No mailboxes") << data << false << QStringList{};
    }
}
...
void DaemonQtTest::fetchMailboxes()
{
    ...
    // Act
    Result<QStringList> result = daemon->fetchMailboxes();
...
void DaemonQtTest::nullptrModules()
{
    ...
    QString setupResult = daemon.setup(Examples::validConfig());
    QString startMonitoringResult = daemon.startMonitoring();
    Result<QStringList> fetchMailboxesResult = daemon.fetchMailboxes();
...
MockPersistentStorage::TestData DaemonQtTest::happyPathStorageTestData() const
{
    ...
    testData.readConfiguration = Result<Configuration>::success(Examples::validConfig());
    testData.writeConfiguration = "";
...
MockMailClient::TestData DaemonQtTest::happyPathMailClientTestData() const
{
    MockMailClient::TestData testData;
    testData.isValid = true;
    testData.configuration = Examples::validConfig();
